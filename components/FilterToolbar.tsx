      ...
      <div className="flex flex-wrap items-center gap-3 px-3 py-2.5">
        <div className="relative">
          ...
          <input
            ...
            className="h-9 w-80 rounded-lg border border-line-2 bg-recessed pl-8 pr-2.5 text-[13px] text-fg outline-none transition-colors placeholder:text-fg-4 hover:border-fg-4 focus:border-accent focus:ring-2 focus:ring-accent/25"
          />
        </div>
        ...
        <button
          ...
          className={`inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-[13px] font-medium transition-colors ${
            open || isFiltered
              ...
              : "border-line-2 bg-recessed text-fg-2 hover:border-fg-4 hover:text-fg"
          }`}
        >
          <SlidersIcon />
          Filters
          {chips.length > 0 && (
            <span className="tnum rounded bg-accent px-1.5 py-0.5 font-mono text-[11px] font-medium text-on-accent">
              {chips.length}
            </span>
          )}
        ...
        <p className="ml-auto text-[12.5px] text-fg-3">
          Showing{" "}
          <span className="tnum font-mono text-[13px] font-medium text-fg">
            {shown}
          </span>{" "}
          of{" "}
          <span className="tnum font-mono text-[13px] text-fg-2">{stats.total}</span>{" "}
          total leads
        </p>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 border-t border-line px-3 py-2">
          ...
          {chips.map((chip) => (
            <button
              ...
              className="group inline-flex max-w-[260px] items-center gap-1.5 rounded-full border border-line-2 bg-surface py-1 pl-2.5 pr-1.5 text-[12px] text-fg-2 transition-colors hover:border-accent/60 hover:text-fg"
            >
              <span className="truncate">{chip.label}</span>
              <span
                aria-hidden="true"
                className="flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full text-fg-4 transition-colors group-hover:bg-accent group-hover:text-on-accent"
              >
                <CrossIcon />
              </span>
            </button>
          ...
          <button
            ...
            className="ml-1 rounded px-2 py-1 text-[12px] font-medium text-accent underline decoration-accent/40 underline-offset-4 transition-colors hover:decoration-accent"
          >
            Clear all
          </button>
        </div>
